...
import { useDeleteInstallationMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteInstallation = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteInstallationMutation();
          ...
          <div className="sm:flex sm:items-start">
            ...
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 className="text-lg font-medium leading-6 text-gray-900">
                {t("deleteTitle", { field: t("Installation") })}{" "}
                <span className="font-semibold text-red-500">{deleteId}</span>
              </h3>
              <div className="mt-2">
                <p className="text-sm text-gray-500">
                  {t("deleteSubtitle", { name: t("Installation") })}
                </p>
              </div>
            </div>
          </div>
