...
import { useDeleteInvoiceMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteInvoice = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteInvoiceMutation();

  const onDelete = async () => {
    try {
      await deleteQuery(deleteId);
      toast.success(t("message.success.delete", { field: t("Invoice") }));
      setOpen(false);
    } catch (error) {
      console.log(error);
    }
