...
import { useEditTripMutation, useChangeTypeOfShipmentMutation } from "../../services/apiSlice";
...
const TypeOfShipment = ({ openTypeOfShipment, setOpen, triggleSubmit, shipment }) => {
    const { t } = useTranslation();
    const [ChangeTypeOfShipment, { isLoading }] = useChangeTypeOfShipmentMutation();
    ...
    const [listTypeOfShipment, setListTypeOfShipment] = useState([
        {
            value: 'Normal'  
        },
        {
    ...
    useEffect(_ => {
        ...
        if (selectedShipment && selectedShipment.shipmentId && openTypeOfShipment) {
            // submitForm1(onSubmit)()
            reset({id: selectedShipment.shipmentId, typeOfShipment: selectedShipment.typeOfShipment})
        }
    }, [openTypeOfShipment, selectedShipment])
    ...
    const onSubmit = async () => {
        if(isValid){
            ...
            try {
                await ChangeTypeOfShipment(transformData);
                toast.success(t("Change type of shipment success"))
                setOpen(false);
                handleClose()
            } catch (error) {
                console.log(error);
                                                ...
                                                <div className="flex min-h-0 flex-1 flex-col">
                                                    ...
                                                    <div className="relative flex">
                                                        <div className="px-4 sm:px-6 w-full h-full">
                                                            ...
                                                            <CustomSelect
                                                                name="typeOfShipment"
                                                                label="typeOfShipment"
                                                                control={control}
                                                                setValue={setValue}
                                                                options={(listTypeOfShipment || []).map((x) => {
                                                                    return { id: x.value, value: `${x.value}` };
                                                                })}
                                                                errors={errors.typeOfShipment}
                                                            required
                                                            />
