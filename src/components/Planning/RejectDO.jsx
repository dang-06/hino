...
import { useRejectDeliveryOrderMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const RejectDO = ({ openRejectDO, setOpen, listReject = [] }) => {
    const { t } = useTranslation();

    const [rejectDO, { isLoading }] = useRejectDeliveryOrderMutation();
    ...
    const onSubmit = async () => {
        try {
            const transformData = { deliveryOrderNumber: listReject }
            const response = await rejectDO(transformData);
                ...
                <div className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                    <div className=" sm:items-start">
                        ...
                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <Box>
                                <h2 className="my-2">List Delivery Order reject: </h2>
                                <div >
